...
class KWinWindowRules:
	...
	def check_kwin_window_rules(app_name: str, app: NotesProtocol):
		print("Checking for KDE Window Rule..")
		...
		if kwin_rules_file.exists():
			...
			if "StickyNote:" in config_text and "simple-sticky-notes" in config_text:
		...
		if kwin_interface.isValid() and bus.isConnected():
			...
			print(" - Window rule not found, offering to add it.")
			reply = QMessageBox.question(None,
					...
					Do you want to add KWin window rule to keep sticky notes from appearing in taskbar and task switcher?
					(This can be done manually, too. See <b>README</b>.)
					""",
					QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
			if reply == QMessageBox.Yes:
				KWinWindowRules._register_kwin_rule(kwin_interface, app)
			else:
				print(" - OK. (Probably) need to add window rule manually!")
	...
	def _register_kwin_rule(kwin_interface: QDBusInterface, app: NotesProtocol):
		...
		try:
			...
			if not kwin_rules_file.exists():
				# No rules file, probably no rules configured. Create one.
				...
				rules_config[new_uuid] = sticky_rules

			else:
				# Need to add a new rule to the existing config file. We want to reference it under [General]-section,
				...
				print(f" - Made backup to {rules_backup_file}")
			...
			# At this point we've either created a new config file or modified the existing one
			# Save changes
